...
def replace_practitioner(combined_dataset_path, id1, id2):
    ...
    files = list(
        filter(lambda x: x.endswith("json"), os.listdir(combined_dataset_path))
    )
    ...
    for fname in files:
        if fname.replace("practitioner", "").startswith(id1):
            ...
            if os.path.exists(J(combined_dataset_path, new_path)):
                complete_obj = None
                with open(
                    J(combined_dataset_path, new_path), "r", encoding="utf-8"
                ) as f:
                    complete_obj = json.loads(f.read())
                    complete_obj["entry"].extend(new_obj["entry"])
                with open(
                    J(combined_dataset_path, new_path), "w", encoding="utf-8"
                ) as f:
...
def merge_big_practitioners(
    combined_dataset_path, n=50, groups=6, limits=(1e5, 1e4, 5e3, 1e3, 5e2, 1e2)
):
    ...
    for group in range(groups):
        ...
        for i in range(n - 1):
            replace_practitioner(
                ...
                sorted_pracs[group * n + i + 1],
                sorted_pracs[group * n],
            )
